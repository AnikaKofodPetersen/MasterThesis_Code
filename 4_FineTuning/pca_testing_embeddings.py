...
def PCA_reduce_stacked_tensor(stacked_tensor, pca_dim=int(args.DIM), esm = args.ESM):
     """
     ...
     if esm == "True":
     	 pca_reload = pk.load(open("pca_esm.pkl",'rb'))
     	 positions_reduced = pca_reload.transform(positions)
     	 explained = None
     else:
         pca_reload = pk.load(open("pca_ps.pkl",'rb'))
         positions_reduced = pca_reload.transform(positions)
         explained = None
...
if __name__ == "__main__":   
        ...
        for part in parts:
            ...
            output =  PCA_reduce_stacked_tensor(stacked_tensor, pca_dim=int(args.DIM))
